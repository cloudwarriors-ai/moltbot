...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
Usage:
  bash scripts/slm-local/smoke-issues-5-6-7.sh [--keep-up] [--keep-gateway-up]
...
existing_memory_pid="$(lsof -ti "tcp:${MEMORY_PORT}" 2>/dev/null || true)"
if [[ -n "${existing_memory_pid}" ]]; then
  kill ${existing_memory_pid} >/dev/null 2>&1 || true
  sleep 1
fi
...
cat >"${GATEWAY_CONFIG_PATH}" <<EOF
{
  ...
  "plugins": {
    "allow": ["slm-pipeline", "slm-supervisor"],
    "load": {
      "paths": [
        "${ROOT_DIR}/extensions/slm-pipeline",
        "${ROOT_DIR}/extensions/slm-supervisor"
      ]
    }
  }
}
...
# run-issues-5-6-7-check prefers API-first category+qa seeding and uses this only for legacy fallback.
smoke_output="$(
  ...
  OPENCLAW_MEMORY_SERVER_URL="${OPENCLAW_MEMORY_SERVER_URL}" \
  OPENCLAW_MEMORY_SERVER_TOKEN="${OPENCLAW_MEMORY_SERVER_TOKEN}" \
  bun "${ROOT_DIR}/scripts/slm-local/run-issues-5-6-7-check.ts"
)"
