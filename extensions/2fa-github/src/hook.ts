...
export function register2FAHook(api: MoltbotPluginApi): void {
  ...
  if (authMode === "browser" && (!githubUsername || !githubPassword)) {
    api.logger.warn("2fa-github: No GitHub credentials configured for browser mode, plugin disabled");
    return;
  }
  if (authMode === "oauth" && (!clientId || !clientSecret || !baseUrl)) {
    api.logger.warn("2fa-github: Missing clientId, clientSecret, or baseUrl for oauth mode, plugin disabled");
    return;
  }
  ...
  async function doBrowserAuth(): Promise<{ success: true; username: string } | { success: false; error: string }> {
  ...
  async function doDeviceAuth(sessionKey: string): Promise<
    | { approved: true; username: string }
    | { approved: false; blockReason: string }
  > {
  ...
  api.on("before_tool_call", async (event, ctx) => {
    ...
    if (existingTrust) {
      api.logger.debug?.(`2fa-github: Trusted session ${sessionKey} (by ${existingTrust.githubLogin})`);
      return;
    }
  ...
  api.registerTool({
    ...
    execute: async (_toolCallId, params) => {
      ...
      if (action === "disable") {
        ...
        return {
          content: [{ type: "text", text: `Trust DISABLED for ${count} session(s). Next sensitive op requires 2FA.` }],
        };
      }
  ...
  api.logger.info?.(`2fa-github: Enabled (mode: ${modeDesc}, tools: ${sensitiveTools.join(", ")}, ${readGateStatus})`);
}
