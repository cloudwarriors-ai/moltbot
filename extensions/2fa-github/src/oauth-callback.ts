  ...
  api.registerHttpRoute({
    ...
    handler: async (req, res) => {
      ...
      if (!pending) {
        res.statusCode = 400;
        res.setHeader("Content-Type", "text/html");
        res.end(renderHtml("Invalid Request", "This authorization link has expired or is invalid."));
        return;
      }
      ...
      try {
        ...
        res.statusCode = 200;
        res.setHeader("Content-Type", "text/html");
        res.end(renderHtml("Approved", `Session authorized as ${githubLogin}. You can close this tab.`));
      } catch (err) {
